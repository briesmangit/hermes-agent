...
class CapacityScorer:
    def __init__(self, tracker: ProviderCapacityTracker, config: PlaneConfig):
        ...
        self._default_weights = config.scorer_weights
        self._provider_weights = config.provider_weights
    ...
    def recompute_all(self) -> None:
        ...
        for key, s in snap.items():
            ...
            # Get provider-specific weights or fall back to defaults
            weights = self._get_weights(s.provider)
    ...
    def _get_weights(self, provider: str) -> dict:
        """Get effective weights for a provider, falling back to defaults."""
        provider_specific = self._provider_weights.get(provider, {})
        if not provider_specific:
            return self._default_weights
        
        # Merge provider-specific weights with defaults
        weights = self._default_weights.copy()
        for key in provider_specific:
            if key in weights and isinstance(provider_specific[key], (int, float)):
                weights[key] = float(provider_specific[key])
        return weights
